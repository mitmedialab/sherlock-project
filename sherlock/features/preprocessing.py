...
def extract_features(data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
    ...
    for raw_sample in tqdm(data):
        ...
        #         random.seed(13)
        ## raw_sample = pd.Series(random.choices(raw_sample_orig, k=n_samples)).astype(str)
        raw_sample = pd.Series(raw_sample).astype(str)

        f = OrderedDict(
            list(extract_bag_of_characters_features(raw_sample).items())
            + list(
                extract_word_embeddings_features(raw_sample, word_to_embedding).items()
            )
