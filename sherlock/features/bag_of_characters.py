...
def extract_bag_of_characters_features(series: pd.Series, features: OrderedDict):
    ...
    char_set = set(''.join(series.str.join('')))
    ...
    for c in CHARACTERS_TO_CHECK:
        ...
        if has_any:
            _mean, _variance, _skew, _kurtosis = compute_stats(counts)
            ...
            features[value_feature_name + '-agg-min'] = np.min(counts)
            features[value_feature_name + '-agg-max'] = np.max(counts)
            features[value_feature_name + '-agg-median'] = np.median(counts)
            features[value_feature_name + '-agg-sum'] = np.sum(counts)
